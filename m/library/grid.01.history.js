...
//-----------------------------------------------
m.request_data_h=function(){
    var limit=parseInt($('#page_size__ID').val());
    var skip=limit*parseInt($('#I__ID').text());
    var mt1=new Date().getTime();
    var c_cmd="count";
    if(m.cmd_t=='s') c_cmd='count-s';
    else if(m.cmd_t=='m') c_cmd='count-m';
    else if(m.cmd_t=='d') c_cmd='count-d';
    else if(m.cmd_t=='p1') c_cmd='count-p1';
    else if(m.cmd_t=='p2') c_cmd='count-p2';
    else if(m.cmd_t=='table') c_cmd='count-table';
    ...
    var f_cmd_h="find";
    if(m.cmd_t=='s') f_cmd_h='find-s';
    else if(m.cmd_t=='m') f_cmd_h='find-m';
    else if(m.cmd_t=='d') f_cmd_h='find-d';
...
m.delete=function(rid){
    var d_cmd="delete";
    if(m.cmd_t=='table') d_cmd='delete-table';
...
//---------------------------------------------
$('#search__ID').on('click',function(){   m.set_req_h(); m.request_data_h(); })
$('#query__ID').on('click',function(){    m.set_req_h(); m.request_data_h(); })
$('#export__ID').on('click',function(){   m.export_records(); })
$('#import__ID').on('click',function(){   m.import_records(); })
$("#p__ID").on('click',function(){  var I=$("#I__ID").text();I--; if(I<0) I=0; $("#I__ID").text(I); m.set_req_h(); m.request_data_h();})
$("#n__ID").on('click',function(){  var I=$("#I__ID").text();I++; if(I>m.max_I) I=m.max_I; $("#I__ID").text(I); m.set_req_h(); m.request_data_h();})
//-------------------------------------
$('#D__ID').on('load',function(){  /*m.input=$vm.vm['__ID'].input;*/ if(m.preload==true) return; if(m.load!=undefined) m.load(); m.set_req_h(); m.request_data_h(); })
//$('#D__ID').on('show',function(){  if($vm.refresh==1){$vm.refresh=0; m.set_req_h(); m.request_data_h();} })
